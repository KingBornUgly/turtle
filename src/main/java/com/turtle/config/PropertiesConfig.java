...
public abstract class PropertiesConfig {
    ...
    private static final Properties loadFromUserDir(String path) {
        ...
        try(final InputStreamReader input = new InputStreamReader(new FileInputStream(file), StandardCharsets.UTF_8)) {
    ...
    private static final Properties loadFromResource(String path) {
        ...
        Properties properties = null;
        try(final InputStreamReader input = new InputStreamReader(PropertiesConfig.class.getResourceAsStream(path), StandardCharsets.UTF_8)) {
            properties = new Properties();
            properties.load(input);
        } catch (IOException e) {
            LOGGER.error("读取配置文件异常：{}", path, e);
    ...
    protected final void persistent(Map<String, String> data, String path) {
        ...
        try(final OutputStreamWriter output = new OutputStreamWriter(new FileOutputStream(file), StandardCharsets.UTF_8)) {
