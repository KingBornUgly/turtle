...
public final class TorrentStream {
    ...
    public void flush() {
        synchronized (this) {
            final ArrayList<TorrentPiece> list = new ArrayList<>();
            this.cachePieces.drainTo(list);
            list.forEach(torrentPiece->flush(torrentPiece));
        }
    }
    ...
    private TorrentPiece cachePiece(int index) {
        ...
        final Iterator<TorrentPiece> iterator = this.cachePieces.iterator();
