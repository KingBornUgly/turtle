...
import com.turtle.model.TorrentPiece;
import com.turtle.model.session.IStatisticsSession;
import com.turtle.model.session.PeerConnectSession;
...
public abstract class PeerConnect implements IPeerConnect {
    ...
    private void requests() {
        LOGGER.info("开始请求下载：{}", this.peerSession);
        ...
        if(this.downloadPiece != null && !this.downloadPiece.completedAndVerify()) {
            LOGGER.info("Piece最后失败：{}", this.downloadPiece);
        ...
        LOGGER.info("结束请求下载：{}", this.peerSession);
    }
