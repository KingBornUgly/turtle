...
import com.turtle.config.UtpConfig;
...
import com.turtle.net.torrent.IPeerConnect;
import com.turtle.net.torrent.peer.PeerCryptMessageCodec;
 ...
 * UTP消息代理
 * @author KingBornUgly
 * @date 2023/1/4 8:55 PM
 * uTorrent transport protocol
 * 协议链接：http://www.bittorrent.org/beps/bep_0029.html
 * UTP头格式
 *   0       4       8               16              24              32
 *   +-------+-------+---------------+---------------+---------------+
...
public final class UtpMessageHandler extends UdpMessageHandler implements IEncryptMessageSender {
    ...
    public void onReceive(ByteBuffer buffer, InetSocketAddress socketAddress) throws NetException {
        ...
        switch (type) {
            ...
            break;
            default : LOGGER.warn("处理UTP消息错误（未知类型）" +
                            "类型：{}" +
                            "扩展：{}" +
                            "连接ID：{}" +
                            "时间戳：{}" +
    ...
    private void state(int timestamp, short seqnr, short acknr, int wndSize) {
        ...
        if(loss) {
            ...
            if(this.ackLossTimes.incrementAndGet() > UtpConfig.FAST_ACK_RETRY_TIMES) {
                // 重新发送最后一个未确认数据包
                final UtpWindowData packet = this.sendWindow.lastUnack();
                if(packet != null) {
