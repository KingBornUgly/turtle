...
import com.turtle.net.torrent.peer.PeerSubMessageHandler;
import com.turtle.net.torrent.peer.PeerUnpackMessageCodec;
...
public final class MSECryptHandshakeHandler {
     ...
     * <p>默认：接收公钥</p>
     *
     * @see MSECryptHandshakeHandler.Step
     */
    private MSECryptHandshakeHandler.Step step = MSECryptHandshakeHandler.Step.RECEIVE_PUBLIC_KEY;
    /**
     * <p>是否加密</p>
     * <p>默认：明文</p>
     */
    ...
    public void handshake() {
        this.step = MSECryptHandshakeHandler.Step.SEND_PUBLIC_KEY;
        this.sendPublicKey();
    }
...
//		Diffie Hellman Yb, PadB
        ...
        if(this.step == MSECryptHandshakeHandler.Step.RECEIVE_PUBLIC_KEY) {
            // 客户端接收连接
            this.sendPublicKey();
            this.step = MSECryptHandshakeHandler.Step.RECEIVE_PROVIDE;
        } else if(this.step == MSECryptHandshakeHandler.Step.SEND_PUBLIC_KEY) {
            // 客户端发起连接
            this.sendProvide();
            this.step = MSECryptHandshakeHandler.Step.RECEIVE_CONFIRM;
        } else {
...
//		ENCRYPT(IA)
        ...
        this.step = MSECryptHandshakeHandler.Step.RECEIVE_PROVIDE_PADDING;
        // PadC IA
...
//		ENCRYPT(VC, crypto_select, len(padD), padD)
        ...
        this.step = MSECryptHandshakeHandler.Step.RECEIVE_CONFIRM_PADDING;
    ...
    private CryptConfig.Strategy selectStrategy(int provide) throws NetException {
        // 客户端是否支持明文
        final boolean plaintext = (provide & CryptConfig.CryptAlgo.PLAINTEXT.provide()) == CryptConfig.CryptAlgo.PLAINTEXT.provide();
        // 客户端是否支持加密
        final boolean crypt = (provide & CryptConfig.CryptAlgo.ARC4.provide()) == CryptConfig.CryptAlgo.ARC4.provide();
        CryptConfig.Strategy selected = null;
        if (plaintext || crypt) {
            // 本地策略
            switch (CryptConfig.STRATEGY) {
                case PLAINTEXT : selected =  plaintext ? CryptConfig.Strategy.PLAINTEXT : null;
                break;
                case PREFER_PLAINTEXT : selected =  plaintext ? CryptConfig.Strategy.PLAINTEXT : CryptConfig.Strategy.ENCRYPT;
                break;
                case PREFER_ENCRYPT : selected = crypt ? CryptConfig.Strategy.ENCRYPT : CryptConfig.Strategy.PLAINTEXT;
                break;

                case ENCRYPT : selected = crypt ? CryptConfig.Strategy.ENCRYPT : null;
                break;
                default : selected = CryptConfig.STRATEGY.crypt() ? CryptConfig.Strategy.ENCRYPT : CryptConfig.Strategy.PLAINTEXT;
            };
        }
        if (selected == null) {
