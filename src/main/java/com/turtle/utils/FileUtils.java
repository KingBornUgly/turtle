...
public final class FileUtils {
    ...
    static {
        // 图片文件
        FILE_TYPE_EXT.put(ITaskSession.FileType.IMAGE, Arrays.asList(
                "bmp", "cdr", "gif", "ico", "jpeg", "jpg", "png", "psd", "svg", "webp"
        ));
        // 视频文件
        FILE_TYPE_EXT.put(ITaskSession.FileType.VIDEO, Arrays.asList(
                "3gp", "avi", "flv", "m3u", "m3u8", "mkv", "mov", "mp4", "mpeg", "mvb", "rm", "rmvb", "wmv"
        ));
        // 音频文件
        FILE_TYPE_EXT.put(ITaskSession.FileType.AUDIO, Arrays.asList(
                "aac", "flac", "m4a", "mp3", "ogg", "wav", "wma"
        ));
        // 脚本文件
        FILE_TYPE_EXT.put(ITaskSession.FileType.SCRIPT, Arrays.asList(
                "asp", "bat", "c", "cmd", "cpp", "h", "java", "js", "jsp", "php", "py", "sh", "sql"
        ));
        // 种子文件
        FILE_TYPE_EXT.put(ITaskSession.FileType.TORRENT, Arrays.asList(
                "com/turtle"
        ));
        // 压缩文件
        FILE_TYPE_EXT.put(ITaskSession.FileType.COMPRESS, Arrays.asList(
                "7z", "bz2", "dmg", "gz", "iso", "jar", "rar", "tar", "z", "zip"
        ));
        // 文档文件
        FILE_TYPE_EXT.put(ITaskSession.FileType.DOCUMENT, Arrays.asList(
                "css", "csv", "doc", "docx", "htm", "html", "json", "log", "md", "pdf", "ppt", "pptx", "txt", "wps", "xls", "xlsx", "xml"
        ));
        // 安装文件
        FILE_TYPE_EXT.put(ITaskSession.FileType.INSTALL, Arrays.asList(
                "apk", "hap", "msi", "exe", "pkg", "dmg", "deb", "rpm", "sis", "sisx"
        ));
        // 未知文件
        FILE_TYPE_EXT.put(ITaskSession.FileType.UNKNOWN, Arrays.asList());
    ...
    public static final ITaskSession.FileType fileType(String fileName) {
        ...
        return FILE_TYPE_EXT.entrySet().stream()
                ...
                .findFirst()
                .orElse(ITaskSession.FileType.UNKNOWN);
    }
    ...
    private static void transferTo(InputStream source,OutputStream target) throws IOException{
        Objects.requireNonNull(target, "out");
