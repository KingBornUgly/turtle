...
public final class PeerUtils {
    ...
    public static final Map<String, Integer> read(List<?> list) {
        ...
        return list.stream()
                ...
                .map(value -> {
                    ...
                    hashMap.put(BEncodeDecoder.getString(map, "ip"),
                            BEncodeDecoder.getInteger(map, "port")
                    );
                    return hashMap;
                })
                .collect(Collectors.toMap(entity -> {
                    return entity.entrySet().stream().findFirst().get().getKey();
                }, entity -> {
                    return entity.entrySet().stream().findFirst().get().getValue();
                }));
    }
    ...
    public static final Map<String, Integer> readIPv4(Object object) {
        ...
        if (object instanceof byte[]) {
            peers = PeerUtils.readIPv4(object);
        } else if (object instanceof ByteBuffer) {
            peers = PeerUtils.readIPv4(object);
        } else if (object instanceof List<?>) {
            peers = PeerUtils.read((List<?>) object);
        } else {
            peers = new HashMap<>();
    ...
    public static final Map<String, Integer> readIPv6(Object object) {
        Map<String, Integer> peers;
        if (object instanceof byte[]) {
            peers = PeerUtils.readIPv6(object);
        } else if (object instanceof ByteBuffer) {
            peers = PeerUtils.readIPv6(object);
        } else if (object instanceof List<?>) {
            peers = PeerUtils.read((List<?>) object);
